  ...
  return (
    <section className="quiet-panel p-5 md:p-8">
      <div>
        ...
        <h2 className="mt-3 text-3xl font-semibold text-zinc-50 md:text-4xl">视觉方向包</h2>
      </div>

      <div className="mt-8 border-l border-cyan-100/22 pl-5">
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {groups.map(([key, title, label]) => (
          <div key={key} className="bg-black/[0.15] p-4">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {(directionPackage[key] as string[]).map((item) => (
                <span key={item} className="border border-white/10 bg-white/[0.025] px-2.5 py-1.5 text-xs text-zinc-400">
                  {item}
                </span>
              ))}
            </div>
