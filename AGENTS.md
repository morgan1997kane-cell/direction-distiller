...
当前版本是 v0.3：优先接入服务端 OpenAI API，并保留本地 mock fallback。
...
本地开发请创建 `.env.local`，写入真实 `OPENAI_API_KEY`。不要提交真实 API key。

如果 `OPENAI_MODEL` 没有设置，默认使用 `gpt-5.5`。如果当前 SDK、账号或环境暂时不支持该模型，可以通过 `OPENAI_MODEL` 切换为可用模型。
