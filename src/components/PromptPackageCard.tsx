    ...
    <section className="bg-[#101216]/80 p-5 md:p-7">
      <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <h2 className="mt-3 text-2xl font-semibold text-zinc-50">Prompt 草稿</h2>
          <p className="mt-3 max-w-2xl text-sm leading-7 text-zinc-500">
            固定提供中文与英文两个版本，便于提案沟通和生图测试。
          </p>
        </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        {active.variation_prompts.map((prompt, index) => (
          <div key={`${language}-${index}-${prompt}`} className="border-t border-white/10 pt-4">
      ...
      <div className="mt-6">
        <p className="mb-3 text-xs text-zinc-500">negative constraints / 限制词</p>
        <div className="flex flex-wrap gap-2">
          {active.negative_constraints.map((item) => (
            <span key={`${language}-${item}`} className="border border-white/10 px-2.5 py-1.5 text-xs text-zinc-500">
              {item}
            </span>
          ))}
        </div>
