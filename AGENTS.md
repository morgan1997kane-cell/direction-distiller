...
当前版本是 v0.3.1：默认接入 DeepSeek 国内可充值供应商，同时保留 OpenAI-compatible 扩展能力和本地 mock fallback。
...
- 可通过环境变量切换到 OpenAI-compatible 供应商
...
v0.3.1 使用 OpenAI-compatible 供应商配置：
...
默认 `AI_PROVIDER=deepseek`。如果 `AI_PROVIDER=deepseek`，读取 `DEEPSEEK_API_KEY` / `DEEPSEEK_BASE_URL` / `DEEPSEEK_MODEL`。如果 `AI_PROVIDER=openai`，读取 `OPENAI_API_KEY` / `OPENAI_BASE_URL` / `OPENAI_MODEL`。
