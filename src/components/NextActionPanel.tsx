...
export function NextActionPanel({ step, result, saved = false, edited = false }: NextActionPanelProps) {
  ...
  const actions = result
    ...
    : [
        "先输入一个 brief，或点击下方示例开始。",
        "可选上传参考图；当前只使用文件名和 metadata 作为文本上下文。",
        "高级用户可展开 AI 引擎设置切换 provider / model。",
      ];
  ...
  return (
    <section className="border-l border-cyan-200/20 bg-cyan-300/[0.035] px-5 py-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        ...
        {result ? (
          <span className="w-fit border border-white/10 bg-black/20 px-2.5 py-1 text-xs text-zinc-400">
