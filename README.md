...
当前版本是 v0.3.1：默认支持 DeepSeek 国内可充值供应商，同时保留 OpenAI-compatible 扩展能力和本地 mock fallback。
...
- Tailwind CSS v4
- localStorage
- OpenAI Node SDK（通过 baseURL 兼容 DeepSeek / OpenAI）
- 服务端 API Route
...
- 当前 v0.3.1 仍不支持参考图真实识别，只把参考图数量、文件名、MIME 类型和 size 作为文本上下文传给 API
