...
当前版本是 v0.3：优先使用服务端 OpenAI API 生成方向包，并保留本地 mock fallback。
...
- `OPENAI_API_KEY` 用于服务端 `/api/generate-direction` 调用 OpenAI API
- `OPENAI_MODEL` 默认是 `gpt-5.5`
- 如果当前 SDK、账号或环境暂时不支持 `gpt-5.5`，可以把 `OPENAI_MODEL` 切换为可用模型
- 不要把真实 API key 提交到仓库
- 如果没有配置 `OPENAI_API_KEY`，前端会自动 fallback 到本地 mock generator
- 当前 v0.3 仍不支持参考图真实识别，只把参考图数量、文件名、MIME 类型作为文本上下文传给 API
